...
const Choices = () => {
    ...
    const handleButton = (text) => {
        // setValue(text)
        navigate("/orgform", {state: {value: text}})
    }
        ...
        <>
            <div className="bg-[#FAF6F5] rounded-lg m-10">
                ...
                <div className="grid grid-cols-4 mx-64 mb-10">
                    {[
                        { icon: BsFillPeopleFill, label: "Wedding", value: "wedding" },
                        { icon: FaBirthdayCake, label: "Birthday", value: "birthday" },
                        { icon: IoSchoolSharp, label: "College Fest", value: "fest" },
                        { icon: MdOutlineFestival, label: "Cultural Event", value: "culture" },
                        { icon: FiPlusCircle, label: "Custom Event", value: "custom" }
                    ].map((item, index) => (
                        <motion.div
                            key={index}
                            whileHover={{
                            ...
                            <h1 onClick={()=>handleButton(item.value)} className="text-2xl font-normal text-center pt-2 mb-9">{item.label}</h1>
                        </motion.div>
                    ))}
                </div>
                <div>
